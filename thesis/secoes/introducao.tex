...
\begin{quote}
Em uma avaliação experimental intraindividual da elevação lateral bilateral,
em que medida um sistema embarcado com IMUs em dois halteres consegue, em
tempo quase real, distinguir padrões corretos de amplitude inadequada,
alterações de cadência, assimetria entre membros e impulso ou balanço, e
quais erros posturais globais permanecem não identificáveis de forma
confiável sem sensores corporais ou câmeras?
\end{quote}
...
As contribuições esperadas deste trabalho incluem:

\begin{itemize}
    \item o desenvolvimento de um protótipo de halter instrumentado com IMUs;
    \item a definição de um protocolo experimental para coleta de dados;
    \item a avaliação da latência para feedback em tempo quase real;
    \item a comparação entre regras manuais, Random Forest, SVM e CNN;
    \item a análise dos limites de identificação de erros usando sensores
    apenas nos halteres;
    \item a discussão sobre a viabilidade de personalização futura por usuário.
\end{itemize}
